      ...
      {requests.map((req) => (
        <div key={req.id} className="rounded-2xl border border-border bg-surface p-5 shadow-xs space-y-3">
          <div className="flex items-center justify-between border-b border-border pb-3">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 text-amber-800 border border-amber-200 px-3 py-1 text-xs font-bold">
              <Clock size={12} />
              {req.status === 0 ? 'قيد المراجعة' : req.status === 1 ? 'تم الرد والتسعير' : 'جاري الصيانة'}
            </span>
          </div>
